...
//   // ADDED: Factory for Map from patient list
//   factory Patient.fromMap(Map p) => Patient(
//     id: p['id']?.toString()?? '',
...
// // ─── APPOINTMENT PAGE ───────────────────────────────────────────────────────
// class AppointmentPage extends StatefulWidget {
//   // ADDED: Optional patient from patient list
//   final Map? preselectedPatient;
//   const AppointmentPage({super.key, this.preselectedPatient});
...
//     _initData();
//     _startQueuePolling();
//     _handlePreselectedPatient(); // ADDED
//   }

//   // ADDED: Auto-fill if navigated from patient list
//   void _handlePreselectedPatient() {
//     if (widget.preselectedPatient!= null) {
//       WidgetsBinding.instance.addPostFrameCallback((_) {
//         if (!mounted) return;
//         final patient = Patient.fromMap(widget.preselectedPatient!);
//         setState(() {
//           _selectedPatient = patient;
//           _patientNameController.text = patient.name;
//           _phoneController.text = patient.phone;
//         });
//         _showSuccess('Patient ${patient.name} selected for booking');
//       });
//     }
//   }
...
class Doctor {
  ...
  factory Doctor.fromJson(Map<String, dynamic> json) => Doctor(
    ...
    name: json['fullName']?? json['name']?? 'Unknown',
    department: json['specialty']?? json['department']?? 'General',
    avatarUrl: json['avatarUrl'],
  );
}
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          _availableSlots = data
            .map((e) => AppointmentSlot.fromJson(e))
            .where((slot) =>!slot.isBooked) // Only show open slots
            .toList();
          _isLoadingSlots = false;
        });
      } else {
            ...
            ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return InkWell(
                  onTap: () => onSlotTap(slot),
                  child: Container(
                    color: isSelected? kTeal.withOpacity(0.1) : null,
                    padding: const EdgeInsets.symmetric(
                      horizontal: 16,
                      vertical: 12,
                    ),
