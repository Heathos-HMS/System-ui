...
class _AdministrativeStaffsPageState extends State<AdministrativeStaffsPage> {
  ...
  final List<AdministrativeStaff> _adminStaffs = [
    AdministrativeStaff(
      id: 'A#001',
      name: 'Rashid Mohammed',
      specialty: 'Administrative Staff',
      joinDate: '07/03/2026',
  ...
  void _onStaffTabSelected(StaffCategory category) {
    if (category == StaffCategory.adminStaffs) return;
  ...
  void _addAdminStaff() {
    // TODO: implement add administrative staff dialog/page
  }
              ...
              children: [
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: _AdminStaffContent(
                      adminStaffs: _adminStaffs,
                      selectedTab: _selectedTab,
                      onTabSelected: _onStaffTabSelected,
                      onAddAdminStaff: _addAdminStaff,
                    ),
                  ),
                ),
                const Footer(),
...
// ─── ADMIN STAFF CONTENT ────────────────────────────────────────────────────
class _AdminStaffContent extends StatelessWidget {
  final List<AdministrativeStaff> adminStaffs;
  final StaffCategory selectedTab;
  final ValueChanged<StaffCategory> onTabSelected;
  final VoidCallback onAddAdminStaff;

  const _AdminStaffContent({
    required this.adminStaffs,
    required this.selectedTab,
    required this.onTabSelected,
    required this.onAddAdminStaff,
  });
      ...
      children: [
        Row(
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: const [
                  ...
                  SizedBox(height: 6),
                  Text(
                    'View and Manage Administrative Staffs.',
                    style: TextStyle(
                      fontSize: 13,
                      color: kTextGrey,
                      height: 1.4,
            ...
            _AddAdminStaffButton(onTap: onAddAdminStaff),
          ],
        ),
        const SizedBox(height: 20),
        _StaffTabs(selectedTab: selectedTab, onTabSelected: onTabSelected),
        const SizedBox(height: 0),
        _AdminStaffsTable(adminStaffs: adminStaffs),
      ],
    );
  }
}

class _AddAdminStaffButton extends StatelessWidget {
  final VoidCallback onTap;
  const _AddAdminStaffButton({required this.onTap});
...
class _AdminStaffsTable extends StatelessWidget {
  final List<AdministrativeStaff> adminStaffs;
  const _AdminStaffsTable({required this.adminStaffs});
    ...
    return Container(
      ...
      child: Column(
        children: [
         ...
         ...adminStaffs.asMap().entries.map((entry) {
            final index = entry.key;
            final adminStaff = entry.value;
            return Container(
              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
              decoration: BoxDecoration(
                color: index % 2 == 0? kWhite : const Color(0xFFF8FCFB),
                border: index < adminStaffs.length - 1
              ...
              child: Row(
                children: [
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child:
                              adminStaff.avatarUrl!= null &&
                                  adminStaff.avatarUrl!.isNotEmpty
                             ? Image.network(
                                  adminStaff.avatarUrl!,
                                  fit: BoxFit.cover,
                  ...
                  Expanded(
                    ...
                    child: InkWell(
                      onTap: () {
                        // TODO: Navigate to administrative staff details
                      },
                      child: Row(
                        children: const [
                          Icon(
